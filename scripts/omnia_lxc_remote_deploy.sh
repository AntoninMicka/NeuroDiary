...
lxc-attach -n "${LXC_NAME}" -- sh -c \
  'export DEBIAN_FRONTEND=noninteractive; apt-get update; apt-get install -y python3 python3-venv python3-pip ca-certificates curl; python3 -m venv /opt/neurodiary/.venv; /opt/neurodiary/.venv/bin/pip install --no-cache-dir -r /opt/neurodiary/backend/requirements.txt'
