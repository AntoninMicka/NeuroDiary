...
NEURODIARY_UPDATE_TRIGGER="${NEURODIARY_UPDATE_TRIGGER:-neurodiary-cloud-pull}"
NEURODIARY_UPDATE_SCHEDULER="${NEURODIARY_UPDATE_SCHEDULER:-neurodiary-cloud-pull}"
...
function configure_github_actions() {
  ...
  if [[ -n "${NEURODIARY_ADMIN_EMAILS}" ]]; then
    gh secret set NEURODIARY_ADMIN_EMAILS --repo "${GITHUB_REPOSITORY}" --app actions --body "${NEURODIARY_ADMIN_EMAILS}"
  fi
  if [[ -n "${NEURODIARY_ADMIN_ALERT_EMAIL}" ]]; then
    gh secret set NEURODIARY_ADMIN_ALERT_EMAIL --repo "${GITHUB_REPOSITORY}" --app actions --body "${NEURODIARY_ADMIN_ALERT_EMAIL}"
  fi
  ...
  gh variable set NEURODIARY_UPDATE_TRIGGER --repo "${GITHUB_REPOSITORY}" --body "${NEURODIARY_UPDATE_TRIGGER}"
  gh variable set NEURODIARY_UPDATE_SCHEDULER --repo "${GITHUB_REPOSITORY}" --body "${NEURODIARY_UPDATE_SCHEDULER}"
...
function deploy_cloud_run() {
  ...
  local env_vars=(
    ...
    "NEURODIARY_UPDATE_TRIGGER=${NEURODIARY_UPDATE_TRIGGER}"
    "NEURODIARY_UPDATE_SCHEDULER=${NEURODIARY_UPDATE_SCHEDULER}"
  )
...
function print_summary() {
  ...
  echo "  NEURODIARY_UPDATE_TRIGGER=${NEURODIARY_UPDATE_TRIGGER}"
  echo "  NEURODIARY_UPDATE_SCHEDULER=${NEURODIARY_UPDATE_SCHEDULER}"
  ...
  echo "  NEURODIARY_ADMIN_EMAILS=<administrator e-mails separated by commas>"
  echo "  NEURODIARY_ADMIN_ALERT_EMAIL=<operations notification recipient>"
  if [[ "${ENABLE_LEGACY_API_TOKEN}" == "true" && -n "${NEURODIARY_API_TOKEN}" ]]; then
